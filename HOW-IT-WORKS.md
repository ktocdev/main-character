...
- **snowflake-arctic-embed-s** (`passages.py`) embeds the passage index. It is
  ...
  part of a long entry can be found, not only its opening. It is downloaded
  once per machine (about 130 MB) to `~/.cache/main-character`.
- **all-MiniLM-L6-v2**, Chroma's built-in model, embeds the journal chunks,
  the summaries and dreams. It reads only the first 256 tokens (about 1,000
  characters) of what it is given, which is why search by meaning moved to
  passages.

The passage index records which model built it. After a model change, or on
an install that has not built it yet, search uses the journal chunks until
`python rebuild_index.py` builds it.
...
| Related history | Search by meaning over the passage index. A long query, such as a whole new entry, is searched piece by piece, and each piece gets its turn at the results, so the ending of an entry counts as much as its opening | 6 passages |
...
as can the passage size and how many neighbouring passages are shown around
each match (`MC_PASSAGE_TOKENS`, `MC_PASSAGE_NEIGHBORS`).
...
3. The text is chunked and embedded into the journal index. This is local and
   free. The passage index does not pick up new entries yet: until the next
   `rebuild_index.py`, a journal whose passage index has been built will not
   find them by meaning. Writing passages here is the next step of the lookup
   upgrade.
