...
class EntryIn(BaseModel):
    ...
    no_reply: bool = False
    # One per deliberate save, generated by the browser and reused when it
    # retries the same attempt: it becomes the entry's id, so a retry of a
    # save that did land is recognised instead of stored twice. Absent (an
    # older client) means the server makes one up and cannot dedupe.
...
@app.get("/api/status")
def status():
    counts = entry_catalog.counts(STATE["collection"])
    return {
        # see INSTANCE_ID: the restart poll watches this, not the status code
        "instance": INSTANCE_ID,
        # Saved and imported journal entries -- not index records. A save
        # adds one at once; a close moves it from open to indexed and leaves
        # the total alone. See entry_catalog for what history counts as.
        "entries": counts["entries"],
        "open_entries": counts["open_entries"],
        "indexed_entries": counts["indexed_entries"],
        # old open-chat writing nobody can classify as saved or sent
        "unclassified_open": counts["unclassified_open"],
        # the raw index size, for anyone who needs it -- never label it entries
        "journal_chunks": STATE["collection"].count(),
...
def _persist_entry(text: str, entry_id: str, when, dream: bool):
    """Store one save: its side effect (the markdown backup, or the dream's
    realm copy) and then the session message that owns it. Both save paths
    come through here, reply and no-reply, so they cannot count differently.

    Returns (saved, dream_id). `saved` is False when this id is already
    stored -- a retry of a save that landed -- and nothing is written again.
    """
    with sessions.LOCK:
...
def write_entry(body: EntryIn, background_tasks: BackgroundTasks):
    ...
    if oversize:
        return oversize
    if body.save_id is not None and not sessions.ENTRY_ID.match(body.save_id):
        return JSONResponse({"error": "malformed save id -- nothing was saved"},
                            status_code=400)
    ...
    saved_headers = {"X-Entry-Id": entry_id, "X-Entry-Saved": "1"}
    ...
    if body.no_reply:
        saved, dream_id = _persist_entry(text, entry_id, when, body.dream)
        if saved:
            ...
            if body.dream:
                import dreams
                _tracked(background_tasks, dreams.ingest_dream_entry, text,
                         dream_id, when)
    ...
    # the entry joins the open session; it becomes journal memory when the
    # chat is closed (the summarize point)
    saved, dream_id = _persist_entry(text, entry_id, when, body.dream)
    if not saved:
        # A retry of a save that already landed. The entry is not stored
        # again, and no second reply is paid for: say it is already saved.
        return JSONResponse({"ok": True, "entry_id": entry_id, "duplicate": True},
                            headers=saved_headers)
    ...
    if body.dream:
        ...
        releases.append(
            _tracked(background_tasks, dreams.ingest_dream_entry, text, dream_id, when))
    else:
        entry_message = (
...
def _install_demo():
    ...
    if SEED_INSTANCE:
        ...
        # read while the locked index survives -- leaving 29 indexed entries
        # on the status line with nothing behind them. Found the hard way.
        return JSONResponse(
...
def data_rebuild():
    ...
    try:
        with _busy():
            ...
            # a rebuild can change which records exist without changing how
            # many there are, which is all the cached count watches
            entry_catalog.refresh(STATE["collection"])
    finally:
        _INDEX_LOCK.release()
    return {"ok": True, **result}
